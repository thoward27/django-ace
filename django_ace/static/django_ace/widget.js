(function () {
  ...
  function apply_widget(widget) {
    var div = widget.firstElementChild,
    ...
    for (var i = 0; i < widget.attributes.length; i++) {
      ...
      var m = attr.name.match(/^data-aceoption-(.+)/);
      if (m) {
        // Get the option name by splitting on '-' and capitalizing the first letter of each part
        // e.g. data-option-enable-snippets -> enableSnippets
        var name = m[1]
          .split("-")
  ...
  if (typeof django == "object" && typeof django.jQuery == "function") {
    ...
    django
      .jQuery(document)
      .on("formset:removed", function (event, $row, formsetName) {
